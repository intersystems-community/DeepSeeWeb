        ...
        .directive('textWidget', ['$parse', '$compile', 'Lang', 'Connector', function($parse, $compile, Lang, Connector) {
            return {
                link: function(scope, element, attrs) {
                    ...
                    element[0].innerHTML =  "<div resize-after-render ng-repeat='item in model.textData' style='display: none; height: {{(100 / (model.textData.length || 1))}}%' class='text-widget-item'><div class='text-widget-label' style='top: {{100 / (model.textData.length || 1) * $index}}%; color: {{item.color}}'>{{item.label}}</div><svg class='text-widget'>"+
