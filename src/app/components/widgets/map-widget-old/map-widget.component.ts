...
export class MapWidgetOldComponent extends BaseWidget implements OnInit, OnDestroy, AfterViewInit {
  ...
  requestPolygons() {
    ...
    // }
    /// url = 'uspolygons.js';
    //this.ds.getFile('/assets/finpolygons.js')
    this.ds.getFile(url)
    ...
    if (feature) {
      ...
      if (this.isWorldMap) {
        ...
        for (let i= 0; i < this.mapData?.Cols[0]?.tuples?.length; i++) {
          const tuple = this.mapData?.Cols[0]?.tuples[i];
          const v = this.mapData?.Data[dataIdx + i]?.toString() ?? '';
          if (v) {
        ...
        title = parts.join('<br>');
        if (title?.toString()) {
          this.showTooltip(title.toString(), e.pixel[0], e.pixel[1]);
        }
      } else {
  ...
  private async loadDefaultGeoJSON() {
    let geo: any;
    try {
      geo = await this.ds.getJSONFile('assets/countries.json');
    } catch (e) {
  ...
  private buildWorldMapPolygons() {
    const geoFeatures: any[] = this.polyData.features;
    const features: Feature[] = [];

    let minV = Number.MAX_VALUE;
    let maxV = Number.MIN_VALUE;
    for (let k = 0; k < this.mapData?.Data?.length; k+=2) {
      const v= this.mapData?.Data[k + 1];
      if (v < minV) {
        minV = v;
      }
      if (v > maxV) {
    ...
    for (let k = 0; k < this.mapData?.Cols[1]?.tuples?.length; k++) {
      const key = this.mapData.Data[k * 2];
      if (!key) {
        continue;
      }
      //const fe = geoFeatures.find(f => f.properties.name === key);
      const fe = this.worldMap?.getFeatures().find(f => f.get('name') === key);
      if (!fe) {
        continue;
      }
      fe.set('dataIdx', k * 2);
      fe.set('title', key);
      fe.set('path', this.mapData.Cols[1].tuples[k].path);
      features.push(fe);
      ...
      const value = this.mapData.Data[k * 2 + 1];
      const v = (value - minV) / (maxV - minV) * 255;
  ...
  private fitPolygonsToScreen() {
    this.map?.getView().fit(this.polys.getExtent(), {padding: [20, 20, 20, 20]});
  }
}
