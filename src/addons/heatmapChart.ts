...
export class HeatmapChartComponent extends BaseChartClass implements OnInit {
    ...
    ngOnInit() {
        ...
        this.chartConfig.tooltip = {
            formatter() {
                ...
                return (_this.chartConfig.yAxis as any).categories[this.point.y] + '<br>' +
                       (_this.chartConfig.xAxis as any).categories[this.point.x] + '<br>Value: <b>' +
    ...
    onHeaderButton(bt: IButtonToggle) {
        if (bt.name === 'showValues') {
            (this.chartConfig.plotOptions.heatmap.dataLabels as PlotHeatmapDataLabelsOptions).enabled = bt.state;
            this.updateChart();
            return;
        }
